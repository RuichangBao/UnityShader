...
Shader "Unlit/SequenceAnimation"
{
	Properties
	{    //括号中前边用来显示在unity中的名字
		_MainTexture("序列帧图片", 2D) = "white" {}   // 序列帧动画纹理
		_Color("颜色", Color) = (1, 1, 1, 1)                        // 颜色
		_HorizontalAmount("行数", float) = 4        // 行数
		_VerticalAmount("列数", float) = 4               // 列数
		_Speed("播放速度", Range(1, 100)) = 30                        // 播放速度
		_aaa("这是我随便写的",int)=100
		//_bbb("这也是我随便写的",bool)="True"
	}
